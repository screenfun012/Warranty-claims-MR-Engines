...
// Auth0 profile ruta na /auth/profile (ne /api/auth/profile)
// useUser hook traži /auth/profile i očekuje 204 kada korisnik nije ulogovan, ne 401
    ...
    if (userId) {
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log('[Profile] Roles from Management API (source of truth):', roles);
          if (rolesFromToken.length > 0) {
            console.log('[Profile] Roles from token (fallback):', rolesFromToken);
          }
        }
      } catch (mgmtError) {
        // Ako Management API ne radi, koristi role iz tokena
        if (process.env.NODE_ENV === 'development') {
          console.warn('[Profile] Could not fetch roles from Management API, using token roles:', mgmtError);
        }
