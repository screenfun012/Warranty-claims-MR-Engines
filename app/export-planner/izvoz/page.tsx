...
const fetchBatches = async (mineOnly?: boolean): Promise<Batch[]> => {
  const url = mineOnly
    ? "/api/export-planner/batches?batchType=MR_ENGINES&mine=1"
    : "/api/export-planner/batches?batchType=MR_ENGINES";
...
const createBatch = async (data: { batchType: string; customName?: string; template?: string }) => {
  const res = await fetch("/api/export-planner/batches", {
    ...
    body: JSON.stringify({ ...data, batchType: "MR_ENGINES", template: data.template || "empty" }),
  });
  if (!res.ok) {
      ...
      <div className="relative p-4 sm:p-6 md:p-8 max-w-6xl mx-auto min-w-0">
        ...
        {isLoading ? (
          ...
          <Card className="p-16 md:p-20 text-center border-2 border-dashed rounded-2xl bg-card/50 dark:bg-card/30 shadow-sm">
            ...
            <h3 className="text-xl font-semibold mb-2 text-foreground">{t("noExportPlanners")}</h3>
            <p className="text-muted-foreground mb-8 max-w-sm mx-auto text-sm leading-relaxed">
              {t("noExportPlannersHint")}
            </p>
            <Button
              onClick={() => setDialogOpen(true)}
              size="lg"
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{t("newExportPlanner")}</DialogTitle>
            </DialogHeader>
            ...
            <DialogFooter>
              ...
              <Button
                onClick={() => createMutation.mutate({ batchType: "MR_ENGINES", customName: newName.trim() || undefined, template: newTemplate })}
